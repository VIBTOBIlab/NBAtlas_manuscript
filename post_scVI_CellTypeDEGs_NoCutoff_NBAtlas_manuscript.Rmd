...
#reduction_to_plot <- params$reduction_to_plot
...
for (celltype in celltypes_levels){
  ...
  table <- FindMarkers(seuratObj, ident.1 = celltype, ident.2 = celltypes_levels[celltypes_levels != celltype], logfc.threshold = 0, min.pct = 0)
...
```{r save_objects}
# save as excel
openxlsx::write.xlsx(FindCellTypeMarkers_results, file = paste0(output_tables, step, "_FindCellTypeMarkers_NoCutoff",  project,".xlsx"), overwrite = T)

# save as rds for reuse
saveRDS(FindCellTypeMarkers_results, file = paste0(output_Robjects, step, "_FindCellTypeMarkers_NoCutoff",  annot_column, "_", project,".rds"))
```
